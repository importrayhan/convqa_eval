...
def main():
    ...
    # ── Build scorer ─────────────────────────────────────────────────────
    scorer = QPPScorer(collection, rewriter_path=args.rewriter_path)
    ...
    if args.threshold_method == "best_f1":
        ...
        best_t, best_train_f1 = find_best_threshold(train_scores, train_labels)
    ...
    # Lower QPP → more ambiguous → higher P(ambiguous)
    # Normalize scores to [0,1] and invert for ambiguous probability
    s_min, s_max = test_scores.min(), test_scores.max()
    if s_max > s_min:
        norm = (test_scores - s_min) / (s_max - s_min)
    else:
        norm = np.full_like(test_scores, 0.5)
    probs = np.column_stack([norm, 1.0 - norm])  # [P(clear), P(ambiguous)]
    ...
    for key in available_keys:
        ...
        if tr_s.max() == tr_s.min():
            continue
        bt, _ = find_best_threshold(tr_s, train_labels)
